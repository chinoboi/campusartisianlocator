      ...
      <View style={styles.topRow}>
        ...
        <View style={styles.info}>
          ...
          <View style={styles.metaRow}>
            ...
            {typeof artisan.rating === 'number' && !Number.isNaN(artisan.rating) ? (
              <View style={styles.ratingRow}>
                <Text
                  style={styles.stars}
                  accessibilityLabel={`Rated ${Number(artisan.rating ?? 0).toFixed(1)} out of 5`}
                >
                  {Array.from({ length: 5 })
                    .map((_, i) => (i < Math.round(Number(artisan.rating ?? 0)) ? '★' : '☆'))
                    .join('')}
                </Text>
                <Text style={styles.rating}> {Number(artisan.rating ?? 0).toFixed(1)}</Text>
              </View>
            ) : (
